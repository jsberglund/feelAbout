...
{
    @IBOutlet weak var taklAboutItView: UIView!
    @IBOutlet weak var feelingView: UIView!
    @IBOutlet weak var aboutCollectionView: UICollectionView!
    @IBOutlet weak var feelingCollectionView: UICollectionView!
    
    //constraints
    @IBOutlet weak var topSpaceForTalkButtonConstraint: NSLayoutConstraint!
...
//        TODO - signin using device ID as unique token/username?
    ...
    @IBAction func talkaboutViewTapped(sender: AnyObject)
    {
        ...
        let submitVC : SubmitViewController = storyboard.instantiateViewControllerWithIdentifier("submitVC") as SubmitViewController
//        myViewController = [[UIStoryboard storyboardWithName:@"Main" bundle:NULL] instantiateViewControllerWithIdentifier:@"someName"];
        
        let myFeeling : FeelingAbout = FeelingAbout(feeling: "Happy", about: "coding", byUser: PFUser.currentUser())
        
        submitVC.feelingAbout = myFeeling
...
//        [self addChildViewController:myViewController];
        ...
        var newConstraint = NSLayoutConstraint(item: self.taklAboutItView, attribute: .Top, relatedBy: .Equal, toItem: self.feelingView, attribute: .Top, multiplier: 1.0, constant: 0)
                ...
                UIView.animateWithDuration(0.25, delay: 0.0, options: .CurveEaseOut , animations: {
                    self.feelingView.removeConstraint(self.topSpaceForTalkButtonConstraint)
                    self.feelingView.addConstraint(newConstraint)
                    self.feelingView.layoutIfNeeded()
                    }, completion: nil)
        
                // 3
                topSpaceForTalkButtonConstraint = newConstraint
